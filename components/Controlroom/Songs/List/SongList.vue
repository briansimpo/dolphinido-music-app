...
function loadMore () {
  if (filter.value.page <= songs.value.meta.last_page) {
    filter.value.page += 1
  }
}
...
<template>
  <div class="list list--lg mt-5">
    ...
    <div v-else>
      <div v-if="songs.data.length > 0" class="row mb-6">
      ...
      <div v-if="songs.data.length > 0" class="row">
        <div class="col-lg-12">
          <div v-for="song in songs.data" :key="song.id">
            <SongListItem :song="song" class="mb-2" />
          </div>
          <div v-if="filter.page <= songs.meta.last_page" class="mt-5 text-center">
